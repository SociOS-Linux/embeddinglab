...
EXPECTED_SERVICE_ID = "embeddinglab.holmes.embedding-pilot"
EXPECTED_FUNCTION = "embedding"
...
def main() -> int:
    ...
    if manifest.get("function") != EXPECTED_FUNCTION:
        return fail("manifest function must be embedding")
    service = manifest.get("service", {})
    if service.get("id") != EXPECTED_SERVICE_ID:
        return fail("manifest service.id mismatch")
    if service.get("ownerRepository") != "SociOS-Linux/embeddinglab":
    ...
    print("OK: embeddinglab manifest and fixtures validated")
    return 0
